...
pNode half(pList p) { 
	...
	while (rabbit != end(p) && rabbit != end(p)->prev) {
...
void push_backN(pList p, int N) {
	...
	for (int i = 0; i < N; i++) {
		int RanNum = rand_extended() % (range+1);
		push_back(p, RanNum);
	}
...
void shuffle(pList p) {
	...
	while (que != nullptr) {
		pNode midNext = mid->next;
		pNode queNext = que->next;
...
bool sorted(pList p, bool (*comp)(int a, int b)) {
	...
	while (curr->next != end(p)) {
		if (!(comp(curr->data, curr->next->data) || (curr->data == curr->next->data))) return false;
		curr = curr->next;
	}
...
void push_sorted(pList p, int val) {
	...
	if (sorted(p, ::less)) insert(more(p, val), val);
	else if (sorted(p, more)) insert(::less(p, val), val);
...
void show(pList p, bool all, int show_n) {
	...
	for (i = 1, curr = end(p); curr != begin(p) && i <= show_n; curr = curr->prev, i++);
