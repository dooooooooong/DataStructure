...
template <typename T>
int binarysearch(const T *data, const T key, int lo, int hi) {
	...
	else if (key > data[mid]) return binarysearch(data, key, mid+1, hi);
...
template <typename T>
T get_a_key(const T *list, int size) {
