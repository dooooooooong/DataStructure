...
tree findBT(tree root, int key) {
	...
	tree target = findBT(root->left, key);
	if (target == nullptr)
		target = findBT(root->right, key);
...
bool findPath(tree node, tree x, vector<int>& path) {
	...
	if (!containsBT(node, x->key)) {
		path.pop_back();
		return false;
	}

	findPath(node->left, x, path);
	findPath(node->right, x, path);
...
tree buildAVL(int* v, int n) {  // recreation method
	...
	for (int i = 0; i < mid; i++) leftv.push_back(v[i]);
	for (int i = mid+1; i < n; i++) rightv.push_back(v[i]);

	root->left = buildAVL(leftv.data(), (int)leftv.size());
	root->right = buildAVL(rightv.data(), (int)rightv.size());
...
tree reconstruct(tree root) {
	...
	if (size(root) > 10) {    // recycling method
		vector <tree> sorted_tree;                      // use new inorder() that returns nodes sorted
		inorder(root, sorted_tree);                      // O(n), v.data() - the array of nodes(tree) sorted
		root = buildAVL(sorted_tree.data(), (int)sorted_tree.size()); // O(n)
	}
	else {                    // recreation method
		vector <int> sorted_key;                      	// use inorder() that returns keys sorted
	    inorder(root, sorted_key);                      // O(n), v.data() - the array of keys(int) sorted
		                      				// clear root 
		root = buildAVL(sorted_key.data(), (int)sorted_key.size()); // O(n)
	}
...
tree trimAVL(tree node, int key) {
	...
	// node with two childeren: replace it with the successor or predecessor
		...
		if (flag) {
			node->key = successor->key;
			node->right = trimAVL(node->right, successor->key);
		}

		else {
			node->key = predecessor->key;
			node->left = trimAVL(node->left, predecessor->key);
		}
	}
