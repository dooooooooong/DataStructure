...
void print_adjlist(graph g){
	...
	for (int i = 0; i < V; i++) {
		cout << "\tv[" << i << "]: ";
		for (gnode index = adj[i].next; index != nullptr; index = index->next) {
			cout << index->item << " ";
		}
		cout << "\n";
	}

	// cout << "adj[0][0]: " << adj[0].item << "\n";
	// cout << "adj[0][1]: " << adj[0].next->item << "\n";
	// cout << "adj[0][2]: " << adj[0].next->next->item << "\n";
	// cout << "adj[0][2]: " << adj[0].next->next->next->item << "\n";
...
void BFS(graph g, int v) {
	...
	while (!que.empty()) {
		int cur = que.front();
		que.pop();  // remove it since processed
...
void DFS(graph g, int v, queue<int>& que) {  
	...
	for (gnode index = g->adj[v].next; index != nullptr; index = index->next) {
		int next = index->item;
		if (!g->marked[next]) {
			g->parentDFS[next] = v;
			DFS(g, next, que);
