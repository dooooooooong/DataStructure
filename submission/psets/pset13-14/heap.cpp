...
void trim(heap p) {
	...
	DPRINT(cout << ">trim\n";);
	int size = p->N -1;
	if (size <= p->capacity/4) reserve(p, p->capacity/2);
...
bool heapOrderedAt(heap p, int k) {
	...
	// According to heap's characteristic, 
	// if node does not have left node, then neither does right node.
	return heapOrderedAt(p, right) && heapOrderedAt(p, left);
}
...
void growN(heap p, int count, bool heapOrdered) {
	...
	randomN(max+1, count, list);

	void(*insertFunc)(heap h, int key) = heapOrdered ? grow : growCBT;

	for (int i = 0; i < count; i++) insertFunc(p, list[i]);
...
// (4. Print the heap if DEBUG is defined whenever a node is deleted.)q
void trimN(heap p, int count, bool heapOrdered) {
	...
	for (int i = 0; i < count; i++) trimFunc(p);
