...
bool insert(unordered_map<string, int>& ht, string key) {
    ...
    it = ht.begin();
    for (auto iter = it; iter != ht.end(); iter++) {
        if (key == iter->first) {
            found = true;
            iter->second++;
        }
    ...
    if (!found) ht.insert(unordered_map<string, int>::value_type(key, 1));

   if (ht.load_factor() >= ht.max_load_factor()) {
        ht.reserve(nextprime(ht.max_load_factor()));
   }

	return true;  // always
}

bool find(unordered_map<string, int>& ht, string key, pair<string, int> &box) {
...
bool erase(unordered_map<string, int>& ht, string key) {
    ...
    unordered_map<string, int>::iterator it =  ht.begin();              // find the key in (index)th list 
...
void show_range(unordered_map<string, int>& ht, vector<int>& bucket_idx, size_t from, size_t to) {
    for (size_t i = from; i < to; i++) {
        cout << "\t[" << bucket_idx[i] << "]  ";
        for (auto x : ht) {
			if (ht.bucket(x.first) == (size_t)bucket_idx[i])
...
int main(int argc, char* argv[]) {
	...
	do {
		...
		case 'f':  // find
			...
			{
				pair <string, int> bucket;
				bool found = find(ht, key, bucket);
