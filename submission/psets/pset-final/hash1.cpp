...
void rehash(Hash*& ht) {
    ...
    int new_size = nextprime(old_size+1);
    ht->tablesize = new_size;
 
    list <string>* newTable = new list<string>[new_size];
    ...
    for (int i = 0; i < old_size; i++) {
        ...
        for (list <string>::iterator key = it; key != ht->hashtable[i].end(); key++) {
...
bool erase(Hash*& ht, string key) {
    ...
    // for loop using it, then return true if erased successfully.
    for (list <string>::iterator iter = it; iter != ht->hashtable[index].end(); iter++) {
        if (key == *iter) {
